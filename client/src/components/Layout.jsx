// ============================================================================
// Prysm — Sidebar Layout Component
// ============================================================================
...
export default function Layout() {
  ...
  return (
    <div style={{ display: 'flex' }}>
      {/* Sidebar */}
      <aside className="prysm-sidebar">
        <div className="prysm-sidebar-brand">
          <FiShield size={28} color="#6366f1" />
          <h2>Prysm</h2>
        </div>

        <ul className="prysm-sidebar-nav">
          {navItems.map((item) => (
            <li key={item.to}>
              <NavLink
                to={item.to}
                className={({ isActive }) =>
                  `prysm-sidebar-link ${isActive ? 'active' : ''}`
                }
        ...
        {/* User info + logout */}
        <div style={{ borderTop: '1px solid var(--prysm-border)', paddingTop: '1rem' }}>
          <div style={{ padding: '0 14px', marginBottom: '0.75rem' }}>
            <div style={{ fontSize: '0.85rem', fontWeight: 600 }}>{user?.name}</div>
            <div style={{ fontSize: '0.75rem', color: 'var(--prysm-text-muted)' }}>
              {user?.phone} · <span className="prysm-badge-accent prysm-badge">{user?.role}</span>
            </div>
          </div>
          <button className="prysm-sidebar-link" onClick={handleLogout} style={{ width: '100%', border: 'none', background: 'none', cursor: 'pointer', textAlign: 'left' }}>
            <FiLogOut /> Logout
          </button>
        </div>
      </aside>
      ...
      {/* Main content */}
      <main className="prysm-main-content">
        <Outlet />
      </main>
    </div>
  );
