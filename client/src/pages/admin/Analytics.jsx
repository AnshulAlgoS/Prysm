// ============================================================================
// Prysm — Enhanced Admin Analytics (Chart.js + Real-Time)
// ============================================================================
  ...
  return (
    <div className="prysm-page prysm-fade-in">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '1.5rem' }}>
        <div>
          <h1 className="prysm-page-title">System Analytics</h1>
          <p className="prysm-page-subtitle">Real-time platform monitoring & disruption predictions</p>
        </div>
        ...
        {kpis.map((k, i) => (
          <div key={i} className="prysm-stat">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
              <div>
                <div className="prysm-stat-value" style={{ fontSize: '1.3rem', background: 'none', WebkitTextFillColor: k.color, color: k.color }}>{k.value}</div>
                <div className="prysm-stat-label" style={{ fontSize: '0.7rem' }}>{k.label}</div>
              </div>
      ...
      <div className="prysm-card">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '1rem' }}>
          {(risk.high_risk_zones || []).slice(0, 6).map((z, i) => {
            ...
            return (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '12px', background: 'rgba(255,255,255,0.02)', borderRadius: 10, border: '1px solid var(--prysm-border)' }}>
                <div style={{ width: 44, height: 44, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: '0.85rem', background: parseFloat(z.risk_score) > 0.7 ? 'var(--prysm-danger-bg)' : parseFloat(z.risk_score) > 0.4 ? 'var(--prysm-warning-bg)' : 'var(--prysm-success-bg)', color: parseFloat(z.risk_score) > 0.7 ? COLORS.danger : parseFloat(z.risk_score) > 0.4 ? COLORS.warning : COLORS.success }}>
                  {riskPct}%
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 600, fontSize: '0.85rem' }}>{z.zone_name}</div>
                  <div style={{ fontSize: '0.75rem', color: 'var(--prysm-text-muted)' }}>{z.city}</div>
                </div>
