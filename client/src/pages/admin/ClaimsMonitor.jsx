  ...
  return (
    <div className="prysm-page prysm-fade-in">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <div>
          <h1 className="prysm-page-title">Claims Monitor</h1>
          <p className="prysm-page-subtitle">Real-time claim tracking and review</p>
        </div>
        <div style={{ display: 'flex', gap: '8px' }}>
          <button className="prysm-btn prysm-btn-primary" onClick={handleForceScan} disabled={loading}>
            <FiPlay size={14} /> {loading ? 'Scanning...' : 'Force Env Scan'}
          </button>
          <button className="prysm-btn prysm-btn-outline" onClick={handleExportCSV}>
            <FiDownload size={14} /> Export CSV
          </button>
          <button className="prysm-btn prysm-btn-outline" onClick={loadClaims}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: '1.5rem', flexWrap: 'wrap' }}>
        {filters.map(f => (
          <button key={f.value} className={`prysm-btn ${filter === f.value ? 'prysm-btn-primary' : 'prysm-btn-outline'}`} style={{ padding: '6px 14px', fontSize: '0.8rem' }} onClick={() => setFilter(f.value)}>
            {f.label}
          </button>
        ))}
      </div>
      ...
      {/* Live indicator */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: '1rem', fontSize: '0.8rem', color: 'var(--prysm-text-muted)' }}>
        <div className="prysm-pulse" style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--prysm-success)' }} />
      ...
      <div className="prysm-card">
              ...
              <tr key={c.id}>
                ...
                <td>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <div style={{ width: 50, height: 6, borderRadius: 3, background: 'var(--prysm-border)' }}>
                ...
                <td><span className={`prysm-badge prysm-badge-${statusColor(c.status)}`}>{c.status?.replace('_', ' ')}</span></td>
                <td>
                  {(c.status === 'under_review' || c.status === 'pending') && (
                    <div style={{ display: 'flex', gap: 4 }}>
                      <button className="prysm-btn prysm-btn-success" style={{ padding: '4px 8px', fontSize: '0.7rem' }} onClick={() => handleReview(c.id, 'approve')} title="Approve">
                        <FiCheck size={12} />
                      </button>
                      <button className="prysm-btn prysm-btn-danger" style={{ padding: '4px 8px', fontSize: '0.7rem' }} onClick={() => handleReview(c.id, 'reject')} title="Reject">
                        <FiX size={12} />
                      </button>
                    </div>
                  )}
        ...
        {claims.length === 0 && <div style={{ textAlign: 'center', padding: '2rem', color: 'var(--prysm-text-muted)' }}>No claims found</div>}
      </div>
    </div>
  );
}
