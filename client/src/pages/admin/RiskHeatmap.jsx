// ============================================================================
// Prysm — Admin Risk Heatmap (Leaflet)
// ============================================================================
...
export default function RiskHeatmap() {
  ...
  return (
    <div className="prysm-page prysm-fade-in">
      <h1 className="prysm-page-title">🗺️ Risk Heatmap</h1>
      <p className="prysm-page-subtitle">Live disruption risk across delivery zones</p>
      ...
      <div style={{ display: 'flex', gap: '1.5rem', marginBottom: '1rem', fontSize: '0.8rem' }}>
        {[
          ...
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ color: 'var(--prysm-text-secondary)' }}>{l.label}</span>
          </div>
        ))}
      </div>

      {/* Map */}
      <div className="prysm-card" style={{ padding: 0, overflow: 'hidden', height: 520 }}>
        <MapContainer
          ...
          <tbody>
            {zones.sort((a, b) => (b.risk_score || 0) - (a.risk_score || 0)).map(z => (
              <tr key={z.id}>
                ...
                <td><span className={`prysm-badge prysm-badge-${z.is_active ? 'success' : 'danger'}`}>{z.is_active ? 'Active' : 'Inactive'}</span></td>
              </tr>
            ))}
          </tbody>
        </table>
