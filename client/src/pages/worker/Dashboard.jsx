// ============================================================================
// Prysm — Enhanced Worker Dashboard (Chart.js + Real-Time)
// ============================================================================
  ...
  return (
    <div className="prysm-page prysm-fade-in">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '1.5rem' }}>
        <div>
          <h1 className="prysm-page-title">{greeting()}, {user?.name?.split(' ')[0]} 👋</h1>
          <p className="prysm-page-subtitle">Real-time insurance dashboard</p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.75rem', color: 'var(--prysm-text-muted)' }}>
            <div className="prysm-pulse" style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--prysm-success)' }} />
            Live {lastUpdated && `· ${lastUpdated.toLocaleTimeString()}`}
          </div>
          <button className="prysm-btn prysm-btn-outline" style={{ padding: '6px 12px', fontSize: '0.75rem' }} onClick={refresh}>
            <FiRefreshCw size={12} />
          </button>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem', marginBottom: '2rem' }}>
        {[
          { label: 'Weekly Coverage', value: stats.activePolicy ? 'Active' : 'None', icon: <FiShield />, color: stats.activePolicy ? COLORS.success : COLORS.warning, bg: stats.activePolicy ? 'var(--prysm-success-bg)' : 'var(--prysm-warning-bg)' },
          { label: 'Claims Triggered', value: stats.claims.length, icon: <FiZap />, color: COLORS.info, bg: 'var(--prysm-info-bg)' },
          ...
          { label: 'Risk Alerts', value: stats.claims.filter(c => c.status === 'pending' || c.status === 'under_review').length, icon: <FiAlertTriangle />, color: COLORS.warning, bg: 'var(--prysm-warning-bg)' },
        ].map((k, i) => (
          <div key={i} className="prysm-stat">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
              <div>
                <div className="prysm-stat-value" style={{ fontSize: '1.5rem', background: 'none', WebkitTextFillColor: k.color, color: k.color }}>{k.value}</div>
                <div className="prysm-stat-label">{k.label}</div>
              </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '1.5rem', marginBottom: '2rem' }}>
        ...
        <div className="prysm-card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            ...
            <span className="prysm-badge prysm-badge-accent">This Week</span>
          </div>
        ...
        <div className="prysm-card">
          ...
          <div style={{ height: 220 }}>
            {stats.claims.length > 0 ? (
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', color: 'var(--prysm-text-muted)', fontSize: '0.85rem' }}>No claims data yet</div>
            )}
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '2rem' }}>
        ...
        <div className="prysm-card">
          ...
          {stats.activePolicy ? (
            <div>
              {[
                { label: 'Coverage', value: <span className="prysm-badge prysm-badge-accent">{stats.activePolicy.coverage_tier}</span> },
                ...
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: i < 4 ? '1px solid var(--prysm-border)' : 'none', fontSize: '0.85rem' }}>
                  <span style={{ color: 'var(--prysm-text-muted)' }}>{r.label}</span>
                  <span>{r.value}</span>
                </div>
              ))}
            </div>
            ...
            <div style={{ textAlign: 'center', padding: '2rem 0' }}>
              <p style={{ color: 'var(--prysm-text-muted)', marginBottom: '1rem' }}>No active policy</p>
              <Link to="/policies" className="prysm-btn prysm-btn-primary"><FiArrowRight /> Buy Policy</Link>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="prysm-card">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
          ...
          <Link to="/claims" style={{ fontSize: '0.8rem', color: 'var(--prysm-accent-light)', textDecoration: 'none' }}>View all →</Link>
        </div>
        {stats.claims.length > 0 ? (
                ...
                <tr key={i}>
                  ...
                  <td>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <div style={{ width: 40, height: 5, borderRadius: 3, background: 'var(--prysm-border)' }}>
                      ...
                      <span style={{ fontSize: '0.7rem', color: 'var(--prysm-text-muted)' }}>{((c.fraud_score || 0) * 100).toFixed(0)}%</span>
                    </div>
                  </td>
                  <td style={{ fontSize: '0.8rem' }}>{new Date(c.created_at).toLocaleDateString()}</td>
                  <td>
                    <span className={`prysm-badge prysm-badge-${c.status === 'auto_approved' || c.status === 'approved' ? 'success' : c.status === 'blocked' ? 'danger' : 'warning'}`}>
          ...
          <p style={{ textAlign: 'center', padding: '2rem', color: 'var(--prysm-text-muted)' }}>No claims yet. Claims are auto-triggered when disruptions occur in your zone.</p>
        )}
      </div>
    </div>
  );
