// ============================================================================
// Prysm — Worker Policies Page (Buy + View)
// ============================================================================
...
export default function Policies() {
  ...
  const statusBadge = (status) => {
    ...
    return <span className={`prysm-badge prysm-badge-${map[status] || 'accent'}`}>{status}</span>;
  };
  ...
  return (
    <div className="prysm-page prysm-fade-in">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
        <div>
          <h1 className="prysm-page-title">My Policies</h1>
          <p className="prysm-page-subtitle">Weekly parametric insurance coverage</p>
        </div>
        <button className="prysm-btn prysm-btn-primary" onClick={() => setShowBuy(!showBuy)}>
      ...
      {/* Buy Policy Form */}
      {showBuy && (
        <div className="prysm-card" style={{ marginBottom: '2rem', border: '1px solid var(--prysm-accent)', background: 'rgba(99,102,241,0.03)' }}>
          <h3 style={{ marginBottom: '1.25rem', fontWeight: 600 }}><FiShield style={{ marginRight: 8 }} /> Buy Weekly Policy</h3>

          {error && <div style={{ padding: 10, background: 'var(--prysm-danger-bg)', borderRadius: 8, color: 'var(--prysm-danger)', fontSize: '0.85rem', marginBottom: '1rem' }}>{error}</div>}

          <form onSubmit={handleBuy}>
            {/* Zone selector */}
            <div style={{ marginBottom: '1rem' }}>
              <label className="prysm-label"><FiMapPin size={12} /> Delivery Zone</label>
              <select className="prysm-input" value={form.zone_id} onChange={e => setForm({...form, zone_id: e.target.value})} required>
            ...
            {/* Disruption type */}
            <div style={{ marginBottom: '1rem' }}>
              <label className="prysm-label">Disruption Type</label>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 8 }}>
                {DISRUPTION_TYPES.map(d => (
                  <label key={d.value} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 12px', borderRadius: 8, border: `1px solid ${form.disruption_type === d.value ? 'var(--prysm-accent)' : 'var(--prysm-border)'}`, background: form.disruption_type === d.value ? 'rgba(99,102,241,0.08)' : 'transparent', cursor: 'pointer', fontSize: '0.85rem' }}>
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              <label className="prysm-label">Coverage Tier</label>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
                {COVERAGE_TIERS.map(t => (
                  <label key={t.value} style={{ textAlign: 'center', padding: '1rem', borderRadius: 12, border: `2px solid ${form.coverage_tier === t.value ? t.color : 'var(--prysm-border)'}`, background: form.coverage_tier === t.value ? `${t.color}10` : 'transparent', cursor: 'pointer', transition: 'all 0.2s' }}>
                    ...
                    <div style={{ fontSize: '0.75rem', color: 'var(--prysm-text-muted)', margin: '4px 0' }}>Payout: {t.payout}</div>
                    <div style={{ fontSize: '0.75rem', color: 'var(--prysm-text-secondary)' }}>Premium: {t.premium}</div>
                  </label>
                ))}
              </div>
            </div>
            ...
            {quote && (
              <div style={{ marginBottom: '1.5rem', background: 'rgba(99,102,241,0.05)', padding: '1rem', borderRadius: 12, border: '1px solid rgba(99,102,241,0.2)' }}>
                <h4 style={{ margin: '0 0 10px 0', fontSize: '0.9rem', color: 'var(--prysm-text)' }}>Dynamic Pricing Breakdown</h4>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {quote.pricing_breakdown?.map((item, i) => (
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.85rem' }}>
                      <span style={{ color: 'var(--prysm-text-secondary)' }}>{item.label}</span>
                      <span style={{ fontWeight: item.amount > 0 ? 500 : 600, color: item.amount < 0 ? 'var(--prysm-success)' : 'inherit' }}>
                        {item.amount > 0 ? '+' : ''}₹{item.amount}
                      </span>
                    </div>
                  ))}
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8, paddingTop: 8, borderTop: '1px dashed var(--prysm-border)', fontWeight: 700 }}>
                    <span>Total Premium</span>
                    <span style={{ fontSize: '1.1rem', color: 'var(--prysm-accent)' }}>₹{quote.premium_amount}</span>
                  </div>
                </div>
              </div>
            )}

            <button type="submit" className="prysm-btn prysm-btn-success" style={{ width: '100%', justifyContent: 'center', padding: 12 }} disabled={loading || !quote}>
      ...
      {policies.length > 0 ? (
            ...
            <tbody>
              {policies.map(p => (
                <tr key={p.id}>
                  ...
                  <td><span className="prysm-badge prysm-badge-accent">{p.coverage_tier}</span></td>
                  <td>₹{p.premium_amount}</td>
                  <td style={{ fontWeight: 600, color: 'var(--prysm-success)' }}>₹{p.payout_amount}</td>
        ...
        <div className="prysm-card" style={{ textAlign: 'center', padding: '3rem' }}>
          <FiShield size={48} color="var(--prysm-text-muted)" />
          <p style={{ color: 'var(--prysm-text-muted)', marginTop: '1rem' }}>No policies yet. Buy your first policy to get protected!</p>
        </div>
      )}
    </div>
  );
