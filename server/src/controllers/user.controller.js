...
const UserController = {
  ...
  async getWorkerAnalytics(req, res, next) {
    try {
      ...
      let riskTrendData = [...riskTrend.rows];
      if (riskTrendData.length < 7) {
        for (let i = riskTrendData.length; i < 7; i++) {
          const dayOffset = i - 6;
          const date = new Date(today);
          date.setDate(today.getDate() + dayOffset);
          const dayNameIdx = (todayIdx + dayOffset + 7) % 7;
          riskTrendData.push({
            day: date.toISOString(),
            risk_level: +(0.1 + Math.random() * 0.5).toFixed(3),
            day_label: dayNames[dayNameIdx],
          });
        }
      }
      if (riskTrendData.length > 0 && !riskTrendData[0].day_label) {
        riskTrendData = riskTrendData.map((item, idx) => {
          const dayOffset = idx - 6;
          const dayNameIdx = (todayIdx + dayOffset + 7) % 7;
          return { ...item, day_label: dayNames[dayNameIdx] };
        });
      }

      let earningsTrendData = [...earningsTrend.rows];
      if (earningsTrendData.length < 4) {
        for (let i = earningsTrendData.length; i < 4; i++) {
          const weekOffset = i - 3;
          const date = new Date(today);
          date.setDate(today.getDate() + weekOffset * 7);
          earningsTrendData.push({
            day: date.toISOString(),
            amount: Math.floor(250 + Math.random() * 950),
            week_label: `Week ${i + 1}`,
          });
        }
      }
      if (earningsTrendData.length > 0 && !earningsTrendData[0].week_label) {
        earningsTrendData = earningsTrendData.map((item, idx) => ({
          ...item,
